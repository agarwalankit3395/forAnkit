...
const sushi_factory_contract = new ethers.Contract(
  SUSHISWAP_FACTORY_ADDRESS,
  IUniswapV2Factory.abi,
  provider
);
const sushi_routerV2_contract = new ethers.Contract(
  SUSHISWAP_ROUTERV2_ADDRESS,
...
const main = async () => {
  ...
  const swapRouterontract = new ethers.Contract(
    UNIV3_SwapRouterAddress,
    SwapRouterABI,
    provider
  );
  ...
  // console.log(poolContract.events)
  poolContract.on(
    "Swap",
    (
      sender,
  ...
  const sushipoolAddress = await sushi_factory_contract.getPair(
    token0_sdk.address,
    token1_sdk.address
  );
  //console.log('sushipoolAddress', sushipoolAddress);

  const sushipoolAddressContract = new ethers.Contract(
    sushipoolAddress,
    IUniswapV2Pair.abi,
    provider
  );
  ...
  const quoteAmountOut =
    await quoterContractUniV3Instance.callStatic.quoteExactInputSingle(
      token0_sdk.address,
      token1_sdk.address,
      3000,
      amountIn.toString(),
  ...
  const calculatePrice_uni = ethers.utils.formatUnits(
